...
class ValidateController extends Controller
{
    public function validateAccount(ValidateRequest $request)
    {
        ...
        ->exists()) {
            return $this->errorResponse(Message::DONT_SEND_EMAIL_VALIDATE_RESET, 403);
        }
        ...
        if ($user) {
            if (!$user->valid) {
                ...
                $this->dispatch(new SendEmailJob($user->email, $user, "SendMailClientCodeActivate"));

                return $this->successResponse([
                    "user_id" => $user->id,
                    "email" => Functions::maskEmail($user->email)
                ], 200, Message::SEND_MAIL_ACTIVATED);
            } else {
    ...
    public function validatePassword(ValidateCodePasswordRequest $request)
    {
        $data = $request->validated();
